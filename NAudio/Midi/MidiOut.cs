...
namespace NAudio.Midi 
{
	...
	public class MidiOut : IDisposable 
	{
		...
		public MidiOut(int deviceNo) 
		{
			...
			MmException.Try(MidiInterop.midiOutOpen(hMidiOut,deviceNo,0,0,MidiInterop.CALLBACK_NULL),"midiOutOpen");
		...
		public MidiOutCapabilities Capabilities 
		{
			get 
			{
				MidiInterop.MIDIOUTCAPS caps = new MidiInterop.MIDIOUTCAPS();
				MmException.Try(MidiInterop.midiOutGetDevCaps(hMidiOut,ref caps,Marshal.SizeOf(caps)),"midiOutGetDevCaps");
				return new MidiOutCapabilities(caps);
			}
		}
